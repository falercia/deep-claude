...
Governança de IA não é a política publicada na intranet que ninguém lê, nem o comitê que se reúne uma vez por ano para registrar que "analisamos os riscos". Governança é o sistema que, em escala, responde a uma pergunta simples: **quando uma saída de Claude causar um problema — um relatório errado, uma decisão equivocada, um dado sensível que não deveria ter entrado no modelo — quem responde?**
...
O Invariante 8 é exatamente essa exigência: dono identificável, em cada saída, em escala. O framework L1-F6 dá a estrutura teórica completa — três camadas, dez controles, 12 decisões mínimas. Este capítulo aplica essa estrutura ao ecossistema Claude: quem pode usar qual plano e feature, como conectores e Skills entram em produção com governança, o que o Admin controla na plataforma, e como calibrar centralização sem matar adoção.
...
Três tiers com implicações de governança distintas: Pro individual, Team organizacional, Enterprise corporativo. O Capítulo 20 detalhou Team e o 19b detalhou Enterprise — aqui o que importa é o que cada tier habilita ou impede em termos de controle.

**Pro individual** não tem administração organizacional. Dados sensíveis podem entrar em contas pessoais sem visibilidade corporativa, sem logs acessíveis ao empregador. Governança depende inteiramente de política publicada e treinada — sem mecanismo técnico de enforcement.
...
**Enterprise** fecha as lacunas que setores regulados precisam resolver: SCIM, offboarding automático, hierarquia de workspaces por departamento, DPA contratual, data residency configurável, VPC isolado, Compliance API com acesso programático a dados de uso para auditoria contínua.
...
Connectors são integrações nativas pré-aprovadas pela Anthropic — Google Workspace, Slack, GitHub e outros. O Capítulo 21 detalhou o funcionamento técnico. A pergunta de governança é: **quem decide o que pode estar conectado, como, e com quais dados?**

Em Team e Enterprise, o Admin controla centralmente quais Connectors os usuários podem ativar. A allow-list garante que Connectors não revisados simplesmente não aparecem como opção — eliminando shadow integrations antes que aconteçam.
...
**2. Quais dados da organização podem transitar por esse Connector?** Um Connector de e-mail pode expor ao modelo dados de clientes, contratos confidenciais, informação financeira. A aprovação deve considerar a classificação dos dados que circulam no serviço conectado.
...
Skills compartilhadas no workspace Team ou Enterprise tornam-se ativos organizacionais acessíveis a todos os membros. A questão de governança é: **quem aprova o que entra no inventário de Skills organizacionais, e como Skills com maior autonomia são controladas?**
...
Skills com scripts executáveis ou acesso a sistemas externos têm perfil de risco diferente. Uma Skill que consulta o CRM, executa análise e distribui relatório automaticamente é, na prática, um agente. Essas Skills exigem governança de agente: dono nominal identificado, revisão antes de entrar em produção, escopo de permissões mínimo, trilha de auditoria de execuções.
...
Para agentes Claude Code (Cap 9) e subagentes (Cap 31): quanto maior o nível de autonomia, mais rigorosos os controles. Agentes com acesso a sistemas de produção requerem RACI assinado, kill switch testado, e gate de revisão no pipeline antes de qualquer promoção de autonomia.

### 42.3.4 — Política de Uso Aceitável e Classificação de Dados

A AUP define o que pode e não pode entrar no Claude. A maioria das organizações publica algo próximo de "use com responsabilidade" e chama de AUP. Uma AUP funcional precisa responder quatro perguntas com clareza:

**O que pode entrar no Claude?** Dados públicos e internos não-sensíveis: sim. Dados pessoais de clientes cobertos por LGPD: depende do plano (Enterprise com DPA habilita com proteções contratuais formais; Pro individual, não). Dados sob sigilo profissional (processo judicial em andamento, M&A não-pública): requer avaliação com jurídico, independente do plano.

**Quem pode usar Claude para qual classe de tarefa?** Redação e análise: todos os colaboradores com licença. Automação de decisões com efeito em sistemas externos: apenas quem tem papel aprovado no RACI para o sistema em questão. Conteúdo confidencial com clientes ou parceiros: requer aprovação do dono da política.

**Quem é o dono de cada saída?** Quando Claude gera um contrato rascunho, quem revisa antes da assinatura? Quando gera análise financeira, quem assina o número? Quando gera código que vai para produção, quem fez o code review? A AUP precisa nomear papéis, não declarar princípios.
...
O L1-F6 define o RACI com 8 papéis e 12 decisões mínimas — template no Apêndice O do L1, para adaptar, não inventar do zero. Decisões que precisam de RACI explícito no contexto Claude:
...
Cada linha do RACI precisa responder: se essa decisão gerar um incidente, quem tem o telefone que vai tocar? Se a resposta for "o time de IA", o RACI não está pronto.

### 42.3.6 — Auditoria e Trilha

Em Enterprise, a Compliance API dá acesso programático a dados de uso e conteúdo de conversas para monitoramento contínuo. Compliance teams integram dados do Claude em dashboards existentes, sinalizam violações automaticamente, e gerenciam retenção com exclusão seletiva. Isso não é apenas feature técnica: é a diferença entre auditoria reativa (investigar depois) e proativa (detectar antes que o erro se propague).

A decisão de retenção é parte da governança. Para setores regulados, reter conversas que geraram decisões com efeito jurídico pode ser obrigatório. Para produtividade geral, retenção estendida aumenta risco de exposição. A Anthropic permite configurar retenção; a organização decide com base em obrigações legais, não em conveniência operacional.

Em Team sem Compliance API, a auditoria se baseia em logs de uso agregados no Admin — suficiente para visibilidade de adoção e custo, insuficiente para auditoria de conteúdo. Se a organização precisa auditar *o que* entrou no Claude em casos específicos, Team sem Compliance API não entrega isso.

### 42.3.7 — O Comitê de IA

O Controle 10 do L1-F6 é explícito: AI Council com mandato e cadência fixa. O comitê não é reunião sobre se IA é boa ideia — é o órgão que aprova mudanças de governança, revisa incidentes, delibera sobre novas superfícies de autonomia e mantém a matriz de maturidade atualizada. O mandato precisa estar escrito — com decisões de competência exclusiva e cadência especificada pelo L1-F6: mensal nos primeiros 12 meses, trimestral após revisão de maturidade.

Pauta tipo para organizações que implantam Claude: revisão de Connectors ou Skills aguardando aprovação, análise de incidentes do período com aprendizados, atualização da allow-list, decisões sobre expansão de autonomia de agentes em produção, atualização da AUP para novos casos de uso.

Sinal de comitê disfuncional: aprovar tudo sem análise (governança de papel) ou nunca aprovar nada (governança que mata adoção). A calibração certa está na próxima seção.
...
A regra de ouro: **centralizar pelo alcance e pela irreversibilidade do risco, não pelo medo genérico de IA.** Redigir uma proposta comercial tem o mesmo perfil de risco que usar um processador de texto. Executar aprovações de crédito em nome da empresa tem o perfil de um sistema financeiro crítico. As duas coisas são "uso de Claude" — e merecem governanças dramaticamente diferentes.

### Como calibrar controle sem matar adoção

Governança que estrangula vira shadow AI: a organização perde visibilidade ao mesmo tempo em que perde controle. O antídoto não é relaxar a governança — é calibrá-la onde necessária e remover atrito onde não é. Três princípios operacionais:

**1. Aprovar categorias, não casos individuais.** O comitê define allow-list de Connectors aprovados, tipologia de Skills que seguem fluxo simplificado vs. revisão completa, e casos de uso de baixo risco automaticamente permitidos pela AUP. Colaboradores operam dentro dessas categorias sem aprovação individual por uso.

**2. Tempo de ciclo de aprovação tem custo.** Se aprovar um Connector leva seis semanas, o colaborador ativa por conta própria ou usa conta pessoal. O processo deve ter SLA declarado (ex.: Connector padrão em 5 dias úteis; Connector com dados sensíveis em 10 dias com DPO), responsável nominado, e critério claro de o que exige revisão aprofundada vs. aprovação expedita.

**3. Visibilidade preferível a bloqueio.** Para uso de baixo risco, é melhor ter visibilidade (logs no Admin) do que bloquear e forçar shadow AI. Visibilidade permite detectar anomalias e corrigir; bloqueio sem explicação não elimina o comportamento — apenas o esconde.
...
O sinal mais importante de todos não está na tabela: **quando alguém pergunta "quem é o dono desse sistema de IA" e a resposta demorar mais de 10 segundos, governança está imatura.** Não importa quantos documentos foram publicados.
...
Uma empresa de serviços financeiros de médio porte — 800 funcionários, financiamento de veículos, carteira de R$ 3 bilhões — decide em setembro de 2025 implantar Claude Enterprise após três meses de piloto com Team em back-office.

A área de compliance identificou três problemas durante o piloto: dados de CPF colados em conversas sem orientação clara, dois Connectors ativados sem revisão de SI (um com acesso amplo ao OneDrive corporativo), e ausência de RACI quando o modelo gerou análise de risco usada diretamente numa decisão de crédito sem revisão humana identificada.

A transição para Enterprise resolveu o problema técnico de visibilidade. O problema de governança exigiu trabalho separado.

O processo levou quatro meses seguindo a estrutura do L1-F6 adaptada:

**Mês 1:** RACI formal com 8 papéis e 14 decisões (incluindo duas adicionadas: "acesso de agente a sistema de crédito" e "uso de Claude em proposta regulatória"). Allow-list de Connectors: 7 aprovados, 3 em avaliação, 1 bloqueado (acesso ao e-mail pessoal corporativo por segregação de dados). AUP de 5 páginas com classificação de quatro níveis de dado e canal de reporte de incidentes.

**Mês 2:** Treinamento de 100% dos usuários em duas horas — foco na AUP e no processo de reporte. Das 12 Skills construídas no piloto, 9 aprovadas com curadoria, 3 desativadas por ausência de dono identificável.

**Mês 3:** Primeiro simulado de incidente. Escalada levou 3,5 horas; objetivo era 1 hora. Gargalo identificado: DPO fora do fluxo de notificação. Corrigido na semana seguinte.

**Mês 4:** Comitê de IA com mandato formal e reunião mensal. Primeira pauta: 3 Connectors aguardando aprovação, análise do simulado, aprovação de nova Skill de análise de financiamento com scripts de consulta ao sistema de crédito (aprovada com condição de revisão humana obrigatória antes de qualquer saída para o comitê de crédito).

Dois meses depois, um analista reportou que Claude gerou análise de risco com dados de outro cliente misturados no contexto — erro de gerenciamento de histórico. Incidente escalado em 47 minutos, registrado como SEV-2, investigado em 24 horas, gerou atualização na política de uso de contexto longo. Nenhum dado saiu da organização. O processo funcionou exatamente porque havia dono, havia trilha, e havia processo de resposta.

Três meses depois, o CFO apresentou a governança de IA ao conselho como elemento da narrativa de gestão de risco para reguladores — não como overhead, mas como maturidade operacional reconhecida.

---

## 42.6 — NA PRÁTICA: TRÊS APLICAÇÕES REPLICÁVEIS

O exemplo anterior descreve um percurso; esta seção entrega o roteiro. Três aplicações que podem ser iniciadas esta semana. Cada uma segue a forma *situação → o que fazer → o ponto de julgamento*, porque o passo a passo é replicável, mas é o ponto de julgamento que separa governança real de governança de papel.

**Aplicação 1 — Aprovar o primeiro Connector com dados de clientes.**
*Situação:* o time quer conectar o Google Workspace ao Claude Enterprise para análise de contratos com clientes. O Connector existe, o plano habilita. *O que fazer:* mapeie o escopo OAuth real do Connector (não o que o nome sugere — o que ele efetivamente solicita); classifique os dados que podem transitar (dado pessoal? confidencial? sensível?); identifique a base legal LGPD aplicável; registre a decisão no RACI com os papéis "quem aprova" e "quem responde" preenchidos por nomes, não por times. *O ponto de julgamento:* decidir se a visibilidade de auditoria via Compliance API cobre o que será processado — ou se é preciso habilitá-la antes de colocar o Connector em uso. Connector aprovado sem trilha de auditoria é o Invariante 8 suspenso no ar: uso acontece, dono desaparece.

**Aplicação 2 — Criar o RACI mínimo para uma Skill com script executável.**
*Situação:* um analista construiu uma Skill que consulta o CRM, gera análise de pipeline e distribui relatório automaticamente por e-mail. A equipe quer publicar para o workspace inteiro. *O que fazer:* identifique o dono nominal (nome, não cargo genérico); documente o escopo de dados acessados pelo script; defina o critério de revisão humana antes da distribuição; estabeleça o kill switch (como desativar em menos de dez minutos se necessário); registre a decisão de publicação com assinatura do tech lead. *O ponto de julgamento:* classificar esta Skill como agente — não como ferramenta conversacional — e aplicar os controles correspondentes. Uma Skill que executa ações no mundo com escopo de dados de terceiros tem o mesmo perfil de risco de um agente autônomo: o fato de ser chamada de "Skill" não muda a mecânica de accountability (Invariante 8).

**Aplicação 3 — Conduzir o primeiro simulado de incidente.**
*Situação:* a organização tem Claude em produção, AUP publicada e RACI assinado — mas ninguém testou o que acontece quando algo dá errado. *O que fazer:* escolha um cenário plausível (ex.: dado pessoal de cliente copiado em prompt por analista sem perceber); ative o fluxo de reporte; meça o tempo até a primeira notificação do DPO; identifique o gargalo; corrija e re-teste. *O ponto de julgamento:* o que o simulado revela sobre quem *realmente* está no fluxo de resposta versus quem *deveria* estar. Governança que nunca foi testada é intenção, não capacidade. O simulado é o termômetro que distingue as duas (Invariante 8: dono identificável, processo ativável).

> 🔧 **EXERCÍCIO**
> Abra o RACI de IA da sua organização — ou admita que ele não existe. Escolha uma das decisões da tabela da seção 42.3.5 que tem o campo "Quem Aprova (A)" preenchido com um time em vez de um nome. Substitua pelo nome da pessoa. Se não conseguir identificar o nome em menos de cinco minutos, você encontrou o gargalo de governança mais caro da sua operação: a decisão sem dono. Documente o nome, confirme com a pessoa, e valide que ela tem o processo de resposta na ponta dos dedos.

---

## 42.7 — CAMADA VIVA

Controles específicos de plataforma, certificações, detalhes da Compliance API, funcionalidades do Admin em Team e Enterprise, escopo de Connectors disponíveis — tudo isso evolui com frequência. Os princípios de accountability, RACI, allow-list, classificação de dados, e comitê com mandato: esses não mudam.

Para os números e detalhes operacionais correntes:
...
Este capítulo trata de governança do ecossistema Claude como plataforma — quem acessa o quê, como conectores e Skills entram em produção, o que o Admin controla. Não substitui:
...
🔗 **Plataforma e planos:** [Team (Cap 19)](L2-C20-team.md) · [Enterprise (Cap 19b)](L2-C20b-enterprise.md) — estrutura de controles por tier, SCIM, DPA, hierarquia de workspaces.
...
🔗 **Operação em produção:** [LLMOps (Cap 35)](L2-C36-llmops.md) — kill switch, rollback automatizado, dono nominal de sistema, governança operacional como complemento da governança de acesso.

🔗 **Autonomia de agentes:** Capítulo de Subagentes e Workflows (Cap 31) — níveis de autonomia e controles proporcionais.

🔗 **Adoção e LGPD:** Capítulo de Adoção e ROI (Parte 4) e Capítulo de LGPD aplicada (Parte 4) — dimensão jurídica e organizacional da implantação.
...
☐ **Validação UAU:** Este capítulo entregou o critério de decisão que o executivo não encontra em documentos genéricos de "melhores práticas de IA" — a distinção operacional entre o que centralizar e o que federar, calibrada ao perfil de risco real de cada superfície do ecossistema Claude, ancorada no Invariante 8 sem duplicar a teoria do L1-F6.
