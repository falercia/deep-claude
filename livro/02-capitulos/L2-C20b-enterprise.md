...
Existe um patamar de maturidade organizacional em que os requisitos de adoção de IA mudam qualitativamente. Bancos, seguradoras, empresas de saúde, governo — todos têm necessidades que vão além do que Team entrega, porque operam sob regimes de compliance que exigem documentação contratual, auditoria formal e controles técnicos que apenas Enterprise provê.

A diferença central — já estabelecida no Cap 19: Team tem SSO via SAML, mas não SCIM. Team tem administração centralizada, mas não hierarquia de workspaces. Team tem proteção de dados por padrão, mas não DPA formal assinável. **Enterprise existe para fechar essas lacunas quando elas são obrigatórias, não opcionais.**

A Anthropic estruturou Enterprise como plano premium customizável, tipicamente contratado por organizações com centenas ou milhares de usuários, preço sob consulta, contratos anuais. A diferença para Team não é incremental em preço — é categórica em governança, segurança e suporte.
...
Certificações como SOC 2 Type II e ISO 27001 são documentadas formalmente e disponíveis para auditoria. No Team, a Anthropic segue boas práticas de segurança, mas sem documentação contratual dessas certificações. Para empresas cujos auditores exigem prova documental, essa é a diferença que desbloqueou o processo.

Enterprise inclui ainda criptografia em trânsito e em repouso (formalizada aqui), VPC isolado via AWS Bedrock para tráfego fora da infraestrutura compartilhada da Anthropic, region selection para localização dos dados, e Data Processing Addendum (DPA) — o documento jurídico que compliance e jurídico precisam antes de assinar qualquer adoção.
...
GDPR, LGPD, CCPA, HIPAA. Inclui retenção customizável, direito ao esquecimento, política de "sem treinamento com seus dados" formalizada **contratualmente** — no Team essa proteção existe por padrão, mas sem contrato que a formalize —, localização de dados por região e relatórios de auditoria.

O ponto mais relevante: **DPA não é documento interno — é contrato bilateral**. Empresas sob GDPR, LGPD Art. 37 ou HIPAA precisam de DPA assinado antes que qualquer dado sensível passe pela plataforma. Team não oferece DPA; Enterprise, sim.
...
**SCIM (System for Cross-domain Identity Management)** é exclusivo do Enterprise. SCIM automatiza criação e remoção de contas conforme o diretório de RH (Workday, BambooHR) ou o provedor de identidade (Okta, Azure AD) é atualizado. Funcionário contratado: conta provisionada. Funcionário que sai: conta desativada — sem depender de Admin lembrar. Para organizações com alto turnover ou offboarding imediato como requisito de segurança, SCIM deixa de ser conveniência e vira controle obrigatório.
...
Uma empresa com múltiplas subsidiárias, BUs ou geografias pode ter workspaces separados com administradores locais, sob políticas globais definidas pelo Admin central. RH não acessa dados de Engenharia. Subsidiária A não acessa projects da subsidiária B. O Admin central vê tudo, com relatórios consolidados.
...
**Aprovação centralizada de MCPs e Skills** fecha o loop de governança: qualquer MCP ou Skill precisa de aprovação do Admin antes de ser disponibilizado ao time. Sem isso, usuários conectam sistemas externos sem supervisão — risco que Enterprise foi desenhado para controlar.

### Pilar 5 — Suporte premium

Customer Success Manager (CSM) dedicado, SLA contratual com uptime garantido, suporte 24x7 para tickets críticos, tempo de resposta por nível de severidade, e engenheiros de solução para integração e troubleshooting. Para uma empresa com 500 pessoas em operações críticas, uma queda de 4 horas sem resposta é incidente sério — o SLA é compromisso contratual, não upgrade de conforto.
...
**Enterprise faz sentido quando requisitos formais que Team não cobre são obrigatórios, ou quando a escala da organização exige administração hierárquica.**
...
Enterprise também pode ser a resposta errada. Uma empresa com 300 usuários, dados não sensíveis, setor não regulado e sem auditoria interna que exija logs detalhados provavelmente opera bem com Team. O sinal de excesso: nenhum item da lista acima é obrigatório — apenas desejável.

A armadilha é contratar Enterprise "por precaução" antes de testar se Team atende. O processo — NDA, revisão jurídica, negociação de SLA, integração de SCIM — leva semanas ou meses. Sem requisitos formais que o justifiquem, esse overhead atrasa a adoção sem adicionar valor.

### O processo de negociação Enterprise

O processo típico começa com contato via página Enterprise da Anthropic, inclui reunião técnica com Account Executive (geralmente após NDA), proposta customizada, revisão jurídica do MSA e DPA, e integração de SCIM e SSO. Setores regulados devem alocar 4 a 12 semanas — o exemplo do banco no próximo item ilustra o extremo.

---

## 19b.4 — EXEMPLO MEMORÁVEL: O BANCO QUE LEVOU OITO MESES PARA APROVAR

Um banco brasileiro de grande porte, com cerca de 12 mil funcionários, decidiu em janeiro de 2025 avaliar a adoção corporativa de IA — começando com Team em 500 licenças para tecnologia e operações. A jornada que se seguiu mostra por que organizações em setores regulados precisam de Enterprise, não Team.

O processo passou por seis instâncias. **Segurança da informação** exigiu SOC 2 documentado, penetration test recente e DPA formal antes de qualquer PoC. **Compliance** verificou conformidade com Bacen, LGPD e política sobre treinamento com dados sensíveis. **Auditoria interna** pediu logs detalhados de uso para amostragem regulatória. **Jurídico** revisou MSA, condições de exoneração e garantias contratuais. **Identidade** exigiu SAML com Okta corporativo, SCIM e MFA obrigatório por política. **Risco operacional** avaliou SLA, plano de continuidade e runbook de incidentes.

Em cada instância, Team falhava em pelo menos um requisito; Enterprise atendia todos com documentação formal. **O contrato foi assinado em outubro de 2025**, oito meses após o início, com 800 licenças e cláusulas customizadas que apenas o plano corporativo permitia.

A lição estrutural: **organizações em setores regulados não podem escolher Team apenas por preço unitário**. O custo de usar plano insuficiente é meses de bloqueio em compliance, retrabalho de implementação e, em alguns casos, impossibilidade legal de prosseguir. **Enterprise existe porque há classes de organização que precisam dele para adotar IA com segurança jurídica — e descobrem isso na primeira tentativa séria.**
